...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        pie_fig = px.pie(spacex_df.groupby('Launch Site')['class'].sum().reset_index(), 
        values='class', 
        names='Launch Site', 
        ...
        select_fig_1 = px.pie(filtered_df,
        values='Unnamed: 0',
        names='class',
...
def get_scatter_chart(entered_site, selected_payload):
    ...
    if entered_site == 'ALL':
        scatter_fig = px.scatter(filtered, x='Payload Mass (kg)', 
        y='class', 
        ...
        select_fig_2 = px.scatter(filtered, x='Payload Mass (kg)', y='class', 
        color='Booster Version Category', title='Correlation between Payload and Success for Site {}.format(entered_site)')
        return select_fig_2
        # return the outcomes piechart for a selected site
